...
YEAR_PAGE_LIMIT = 1000  # この枚数を超える年は月別ページに分割
...
def organize_photos(root: Path, photos: list[Path]) -> list[Path]:
    """写真をEXIF撮影日に基づいて YYYY/MM/ フォルダへ移動する。"""
    moved = 0
    skipped = 0
    new_photos = []
    gallery = (root / GALLERY_DIR).resolve()
    ...
    for photo in photos:
        ...
        dest_dir = root / str(date.year) / f"{date.month:02d}"

        # すでに正しいフォルダにある場合はスキップ
        try:
            photo.relative_to(dest_dir)
    ...
    print(f"  → {moved} 枚を移動しました", end="")
    if skipped:
        print(f"（{skipped} 枚は整理済み）", end="")
    print()
    return new_photos
...
def generate_year_month_selector_html(
    year: int, month_info: list[dict], gallery: Path, title: str,
) -> Path:
    """1000枚超の年用 — 月選択ページを生成。"""
    total = sum(mi["count"] for mi in month_info)
...
<header class="hdr" style="position:relative">
  <a class="back" href="index.html" style="position:absolute;left:16px;top:50%;transform:translateY(-50%);font-size:.8rem;color:var(--dim)">&#8249; 戻る</a>
  <h1>{year}</h1>
  <p class="sub">{total:,} Photos &mdash; {h(title)}</p>
</header>
<div class="yg">
...
def main():
    ...
    # 写真整理オプション
    print("  撮影日でフォルダ整理しますか？")
    print("  （EXIF日付に基づいて YYYY/MM/ フォルダへ移動します）")
    ans = input("  [y/N]: ").strip().lower()
    if ans in ("y", "yes"):
        print()
        print("  写真を整理中…")
        photos = organize_photos(root, photos)
        print()
    ...
    for year in sorted(by_year.keys(), reverse=True):
        ...
        if len(yr_photos) > YEAR_PAGE_LIMIT:
            # 月別に分割
            by_month: dict[int, list[dict]] = defaultdict(list)
            for p in yr_photos:
                by_month[p["m"]].append(p)
            ...
            print(f"    {year}: {len(yr_photos):,} 枚（{len(month_info)} 月に分割）")
        else:
